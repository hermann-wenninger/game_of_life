...
function start() {
  ...
  gsap
    ...
    .fromTo(
      ".dot",
      { scale: 0.5, rotate: 0, opacity: 0, svgOrigin: "50 50" },
      {
        scale: 1,
        opacity: 1,
    ...
    .fromTo(
      "#bot, .stars use",
      { opacity: 0, scale: 1 },
      { opacity: 1, stagger: 0.05, ease: "expo" },
      2
    )
    .fromTo(
      ".ant",
      { y: 3 },
      { y: 0, transformOrigin: "50% 100%", ease: "back.out(2)" },
      2.2
    )
    .add(removeDot)
    .fromTo(
      "#timer",
      { transformOrigin: "50% 100%", rotate: 360 },
      {
        rotate: 0,
        ease: "none",
        duration: 15 / (level / 5),
    ...
    if (botPos.x == starPos.x && botPos.y == starPos.y) {
      ...
      gsap
        ...
        .fromTo(
          ".ant",
          { y: 0 },
          { duration: 0.3, y: 1.5, yoyo: true, repeat: 1, ease: "back.in(3)" },
          0
        )
        .fromTo(
          ".mouth",
          { attr: { d: "M3,6.8L4,6.8L6,6.8L7,6.8" } },
          {
...
function end(gameOver = false) {
  if (nStars == 3 || gameOver) {
    ...
    gsap
      ...
      .to(
        ".markers use",
        { attr: { fill: "#000" }, ease: "power2.inOut", stagger: -0.1 },
        "-=0.5"
      )
      .set("#grid, .txt1", {
        attr: {
          fill: gameOver ? "#fff" : "hsl(" + (150 + level * 55) + ",100%,50%)",
        },
      })
      .fromTo(
        ".txt1",
      ...
      .add(() => {
        if (!gameOver) start();
          ...
          gsap
            ...
            .to(
              ".txt2, .txt3, #replay",
              { autoAlpha: 1, stagger: 0.15, ease: "power2.inOut" },
              "-=0.25"
            );
        }
      });
